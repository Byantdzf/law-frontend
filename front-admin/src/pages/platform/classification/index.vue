  ...
  export default {
        ...
        columns: [
          {
            ...
            propsHandler ({ col, row }) {
              return {
                col,
                row,
                type: row[col.prop] == 1 ? 'danger' : 'success'
              } 
            }
          },{
            label: '更新时间',
    ...
    methods: {
      ...
      async formSubmit(form) {
        try {
          if ('id' in form) {
            await this.categoryUpdate(form)
            this.closeDialog()
            this.refreshTable()
            this.$msgSuccess('修改成功')
          } else {
            await this.categoryAdd(form)
            this.closeDialog()
            this.refreshTable()
        ...
        try {
          switch (type) {
            ...
            case 'edit':
              ...
              break;
            case 'del':
              await this.$confirm('确认删除此分类吗?', '温馨提示', { type: 'warning' })
              await this.categoryDel(row.id)
      ...
      async handleMultiDel() {
        if (this.tableSelected.length) {
          try {
            ...
            await this.$confirm('确认删除选中的分类吗?', '温馨提示', { type: 'warning' })
            await this.categoryDel(ids)
            this.$msgSuccess('操作成功！')
            this.refreshTable()
          } catch (error) {
